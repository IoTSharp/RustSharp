...
P1-02 发布第 3 版 `safe-core-syntax`：49 个夹具、18 个必需类别及 34 份精确 AST
快照，详见[语法契约](docs/syntax-profile.md)。解析器现覆盖递归导入、受限可见性、
外部模块声明、文档注释脱糖及内部属性、生命周期/类型/const 泛型及默认值、`where`、
trait/impl 和关联项、函数及约束类型、限定路径、结构体/枚举形态、字段/方法访问、
cast/range/`?`、丰富模式、`match`、循环、闭包及 `let`-`else`/let-chain。
取消、词法/语法共享超时、错误恢复，以及集合、操作数和嵌套深度限制继续生效。

清单和报告记录 Rust 1.98.0 / Edition 2024、类别/用例 ID、源码/清单/快照哈希、
解析器限制和预期结果。每个解析成功用例必须匹配随仓库保存的 AST 快照，包括区分
语法的字段和源码范围；每个解析失败用例必须匹配诊断代码及精确源码文本。回归覆盖
AST 结构、格式错误输入、取消、资源耗尽、六种非法契约变异和三种错误预期变异，
其中包含错误 AST 快照。Windows/Linux CI 使用 `eng/Test-SyntaxEvidence.ps1`
验证当前证据；这份本地记录不宣称新的远程 CI 运行已通过。

其 P1-01 依赖为 ✅ 已完成。语法契约明确排除 unsafe/FFI、async/await、static/union
项及通用宏。外部模块加载属于 P1-03。当前名称解析/HIR 配置无法表示的新语法会返回
`RSN1007`，包括嵌套泛型约束、匿名 `const` 项、表达式/模式中的绝对路径，以及尚不支持的
结构化 AST 扩展和非文档 Rust 属性。下方 P1-03 模块批次实现了此前待办模块语法的
有界子集。诊断代码和源码范围会经 HIR 降低保留到编译器检查/编译
结果；编译会在写入输出产物前拒绝这些输入。语法验收不代表语义、rustc 差分或
运行时一致性；完整 P1 门槛仍为 🚧 进行中。

此前 2026-09-08 的本地 P1-02 闭环证据，环境为 Windows x64、.NET SDK 10.0.400/runtime
10.0.11：✅ 已完成，Release 构建零警告/错误、141/141 项可执行回归、49/49 项语法
用例、34/34 份 AST 快照、18/18 类映射、24/24 项词法用例、6/6 项名称解析及对照
rustc 1.98.0 的 14/14 项基础类型差分。PowerShell 7 检查器接受当前证据，并拒绝
过期清单/快照哈希。报告位于 `artifacts/conformance/`；差分报告为
...
和 NFC 规范化标识符，以及明确的工作量限制。其十个基线本地测试工具用例覆盖命名空间/
符号收集、导入和限定路径、重复/歧义名称、可见性和未解析名称、导入循环、声明顺序和
合法遮蔽、拒绝对函数局部变量进行限定访问、结构体字段和枚举泛型参数、Unicode 规范化、
Unicode 补充平面标识符，以及符号/导入嵌套限制。当前 25 用例 `safe-core-name-resolution`
清单扩展了此前六用例的有界验收基准集合。`SafeCoreHirLowering` 还将成功的树降低为确定性的
平面 arena，其中包含已绑定的声明/引用符号；四个测试工具用例覆盖确定性 ID、代表性
节点形态、依赖失败、Unicode 等价绑定和明确的工作量限制。这些属于前端证据；基础
类型配置现在已将这些阶段接入编译器。

P1-03 模块批次支持嵌套分组和 glob 导入、分组中的 `self` 别名、匿名 `_` 导入，以及受限的
`pub(crate)`、`pub(self)`、`pub(super)`、`pub(in ancestor)` 可见性，且重导出不能扩大
可见范围。基础类型配置的
`CheckFile`/`CompileFile` 和 CLI 文件命令现会从 `foo.rs` 或 `foo/mod.rs` 加载已声明
模块，保留原文件诊断和 Portable PDB 校验和/函数位置。加载有界，并在输出前拒绝缺失、
歧义、无效或不安全的模块路径；所有已加载源码都会与输出路径进行冲突检查。字符串
API 仍以 `RSN1007` 拒绝外部模块。前导 `::` 仍返回 `RSN1007`，因为 Edition 2024
外部预导入表仍是明确边界。P1-03 对声明的语义/HIR 和本地包配置档已 ✅ 已完成；更广泛的属性、feature 和注册表包支持仍在计划中。
[模块契约](docs/module-profile.md) 定义精确的布局、诊断、预算及剩余工作。

来自模块/枚举的 glob 导入现通过有界固定点解析收敛，包括传递重导出和循环。局部声明
与显式导入按命名空间遮蔽 glob；限定路径前缀在类型命名空间中查找；相同规范目标会
合并，歧义在使用名称时诊断，导入可见范围取其与目标可见范围的交集。
HIR `ImportGroup` 节点保留展开后的绑定。
显式 `use` 导入独立解析类型和值命名空间，包括别名/重导出链。同一规范目标对外合并为
一个 `Both` 绑定；不同目标则降低为 `ImportGroup` 中的独立绑定。缺失的命名空间分支
不会制造歧义，也不会遮蔽该命名空间中的 glob。所有导入都参与有界固定点解析。
源码 `///`、`//!`、`/** ... */`、`/*! ... */` 注释在根/项/模块/代码块/字段/变体 HIR
中保留源码范围、内部附着位置及 `DocumentationAttribute`。基础类型执行会惰性忽略
这些文档元数据。显式 `#[doc]`、未知根/项属性，以及参数/泛型参数属性仍以 `RSN1007`
拒绝；不支持的根/项属性指向精确的属性源码范围。

此前 2026-09-08 的文件模块批次验证为 ✅ 已完成：Release 构建零警告/错误、171/171 项
可执行回归，其中包括 12 项工作区、7 项模块名称解析、5 项模块编译和 2 项 PDB 测试。
语法验收通过 49/49 个用例（34/34 份 AST 快照、18/18 类映射）及 PowerShell 证据
检查器；名称解析通过 6/6 个用例。原有基础类型差分套件本轮对照 rustc 1.98.0 重跑，
通过 14/14 个用例。CLI 检查/编译成功，生成的模块示例输出 `42` 和 `true`；独立
ILVerify 验证通过。报告位于 `artifacts/p1-03/`：`safe-core-syntax.json`、
`safe-core-name-resolution.json`、`safe-core-primitives-v1.json` 和
`modules.ilverify.json`。另有 12 项模块规则的 rustc 元数据编译抽查作为补充证据，
不代表完整模块差分清单。P1-03 对声明的配置档已 ✅ 已完成；更广泛的差分覆盖仍在计划中。

此前 2026-09-08 的 glob/文档注释续批对该次增量为 ✅ 已完成：Release 构建零警告/错误、
180/180 项可执行回归，其中包括 12 项工作区、13 项模块名称解析、7 项模块编译和
2 项 PDB 测试。九项新增测试覆盖六项 glob 回归、一项属性边界回归和两项 HIR/执行
...
当前名称解析清单从六用例基线扩展为 25 个用例，包含精确绑定和诊断范围预期。新增
分组/self/匿名导入、glob 优先级与歧义、固定点导入链与循环、受限可见性、独立类型/值
导入、文档注释，以及绝对导入和未求值属性的 `RSN1007` 拒绝契约。此前 6/6 报告仅作为原有六用例的证据。本次增量 ✅ 已完成：Release 构建零警告/错误，可执行回归通过 186/186，扩展清单通过 25/25，PowerShell 7 有界证据检查器验证通过。完整模块差分及语义/HIR 基准集合仍待实现。

### P1-04 类型分析配置档

可选的 `safe-core-types-v1` 配置档为已声明的全部基础数值类型、元组、数组、切片、
引用、函数指针、非泛型 ADT、别名和 never 增加类型检查，包括模式/match、闭包、
有界 const 求值、推断和有方向的强制转换。
...
第 2 版验收语料覆盖十六个必需类别中的 96 个用例，每类均包含正向和反向用例。
验收强制检查类别归属、必需用例 ID 和精确失败代码/范围。
P1-04 对这一已声明的单态类型契约为 ✅ 已完成。泛型/trait 推理、类型化 MIR、所有权/生命周期及可执行降低
各自保留独立里程碑，见 [ADR 0008](docs/adr/0008-safe-core-type-checking.md)。

2026-09-17 记录的证据：265/265 项可执行回归、更新后的类型系统示例检查，以及全部
十六个必需类别中的 96/96 项第 2 版差分用例（60 项编译通过、36 项编译失败）均为
✅ 已完成；失败和跳过均为零。
报告为 `artifacts/p1-04/safe-core-types-v1.json`，记录
`rustc 1.98.0 (88d9e12ae 2026-08-18)`、.NET 运行时 10.0.12、`win-x64`，无清理诊断，
也未遗留自有测试/运行目录。Release 构建零警告/错误。本地验证显式使用已安装的 .NET SDK
...
`p1-differential-v1` 报告记录四次 rustc 1.98 oracle 通过和四次 RustSharp 源码级失败，
且没有跳过：借用夹具停在 `RSM2002`，Drop 夹具停在 `RSN1007`。该报告保留为明确
缺口，不会被改写成通过结果。
...
证据为 ✅ 已完成。P1-06 至 P1-10 现已有有界实现基础和 452/452 项可执行测试证据；
已记录的 P1-10 报告通过 8/8 和 24/24 个回归用例。已记录的 `p1-exit-gate-v1` 报告通过
5/5 个进程内库契约探针，并记录 `"nativeAot": false` 和 `"crossPlatform": false`；
它不能关闭完整 P1 退出门槛。它们仍为 🚧 进行中，因为完整的引用/模式/闭包语义、
编译器集成的析构/Drop 降低、更完整的声明合成、运行时/AOT 门槛和完整差分分母尚未满足。
