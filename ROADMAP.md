...
| P1-10 | 🚧 In progress | [Leaf breakdown](docs/roadmap/P1.md#p1-10). Establish compile-pass, compile-fail, run-pass, and differential regression suites. | P0-11, P1-06.01 | `dotnet run --project tools/RustSharp.Conformance -c Release --no-build --no-restore -- --profile safe-core-regression-v1 --oracle rustc-1.98 --report artifacts/p1-10/safe-core-regression-v1.json`<br>`dotnet run --project tools/RustSharp.Conformance -c Release --no-build --no-restore -- --profile safe-core-regression-v3 --oracle rustc-1.98 --report artifacts/p1-10/safe-core-regression-v3.json --timeout 30 --deadline 300`<br>`dotnet run --project tools/RustSharp.Conformance -c Release --no-build --no-restore -- --profile p1-differential-v2 --oracle rustc-1.98 --report artifacts/p1-10/p1-differential-v2-current4.json --timeout 30 --deadline 300` | The immutable regression v1 and v2 manifests retain 8 and 24 cases; historical reports pass 8/8 and 24/24. The new regression v3 fixes 26 cases (1 compile-pass, 4 compile-fail, 17 run-pass, 4 differential), versions two now-executable pattern/capture expectations and adds combined MIR family/projection samples. The borrow/Drop v2 corpus remains 16 cases (10 borrow, 6 Drop). The workflow runs 12 CoreCLR/ILVerify/Native AOT cases plus regression v3 on each native x64 platform and validates six reports; the aggregator also preserves explicit v2 report selection. Historical [CI run 35848782833](https://github.com/IoTSharp/RustSharp/actions/runs/35848782833) passed its six inputs at `23279d93267a814c643baddc29c72918ff0fda0b` and cannot validate later additions. The full requirement-to-case ledger, expanded borrow/Drop/platform suites and final-SHA evidence remain open. |
...
P1-05's bounded executable generic contract and P1-06's frozen typed-MIR
implementation are ✅ Complete on their recorded evidence. Continue the source
ownership, generated cleanup, package contracts and expanded evidence tracks.
Each PR states its exact delivery, evidence and remaining milestone criteria.

P1-06.01's [exit scope ledger](docs/p1-exit-scope-v1.md) retains 40 requirement IDs
and their owning leaves. All 19 P1-06 leaves are ✅ Complete; the
[implementation inventory](docs/p1-06-implementation.md) maps the executable
families to their registered tests. The [family sample](samples/mir-families.rs)
and [projection sample](samples/mir-places.rs) match rustc 1.98.0 on CoreCLR and
Windows x64 Native AOT and pass ILVerify 10.0.11 without suppressed diagnostics;
GC-owned references resolve the former `ReturnPtrToStack` diagnostic. The historical
[storage sample](samples/mir-places-verified.rs) evidence is retained.
...
evidence. P1-06 is ✅ Complete for the frozen typed-MIR contract, with 670/670
local executable harness regressions, regression v3 26/26, borrow/Drop v2 16/16,
zero failures/skips and a zero-error/zero-warning Release build. The recorded `p1-exit-gate-v1` report passes 5/5 in-process
library-contract probes and records `"nativeAot": false` and
`"crossPlatform": false`; it does not close the full P1 exit gate. P1-07 through
P1-10 and the P1 stage remain 🚧 In progress for the complete source ownership,
emitted destructor-failure/panic behavior, source-level imported contracts and
expanded differential/native-platform evidence.
