...
| P1-06 | 🚧 In progress | [Leaf breakdown](docs/roadmap/P1.md#p1-06). Define typed MIR, CFG validation, desugaring, and source mapping. | P1-04 | `dotnet run --project tests/RustSharp.Tests -c Release --no-build --no-restore` | The bounded implementation provides immutable typed MIR arenas; strict CFG/type/constant/operator/call validation; deterministic bounded formatting and snapshots; nested tuple and fixed-array aggregate rvalues with statically checked indexing; structural-`Copy` repeated-array lowering; tuple/scalar patterns, guards, or-pattern CFGs and statically expanded closures in the opt-in v2 profile; HIR lowering with source spans; bounded source-to-MIR ownership/cleanup evidence; direct MIR-to-CLR-LIR lowering for the supported value subset; and cancellation, work, depth, collection and diagnostic limits. Local full-array slice references now support unsizing, length and constant indexing through proven owner storage, with deterministic MIR and CoreCLR regressions. Dynamic indices, subslices, slice writes/parameters/returns, full place provenance, broader runtime/AOT coverage and the full P1 exit gate remain open; see the [typed MIR contract](docs/typed-mir-profile.md). |
...
Start with P1-06.01 and P1-10.01/.02 to freeze the requirement/case inventory.
Then use the exact leaf dependencies in the [P1 execution order](docs/roadmap/P1.md):
