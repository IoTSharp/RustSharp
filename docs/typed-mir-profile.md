...
value HIR-to-MIR boundary, including nested tuple rvalues. P1-06 remains open for
arrays, references, closures, match, const, move, and drop lowering and for later
ownership and backend integration.
...
Functions have unit, bool, char, integer, floating-point, or bounded tuple
parameters and return values. Tuple elements may themselves be bounded tuples;
tuple arity and nesting remain subject to the lowering limits. A function may
also return never (`!`). Scalar aliases, modules, and resolved imports keep their
existing HIR meaning. Calls resolve directly to a function item in the same HIR
document, including forward and recursive calls.
...
Bounded tuple values and tuple rvalues are supported, including nested tuples.
Arrays, references, function-pointer values/indirect calls, closures, `match`,
destructuring, `let-else`, const items, and inline const blocks produce `RSM2002`
at the unsupported construct. Tuple projections and destructuring are not part
of this boundary. Unreachable source tails after an unconditional transfer are
omitted after P1-04 has checked their types; they are not represented as
executable MIR or independently checked against this subset. No unsupported
construct is translated into a dummy value.
...
rvalue types (including tuple element types), supported operations, function/call
...
It checks deterministic typed snapshots, scalar and nested-tuple constants/rvalues,
HIR provenance, exact unsupported spans, and work/depth/size/time/cancellation
failures. A small,
