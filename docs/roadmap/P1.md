...
| E7 | Current struct/place/reference subset: `SafeCoreMirAdtLayoutTests`, `SafeCoreMirAdtSourceTests`, `SafeCoreMirPlaceTests`, `SafeCoreMirProjectionBackendTests`, `SafeCoreMirReferenceProvenanceTests` and `SafeCoreMirSliceTests`. [Projection sample](../../samples/mir-places.rs) produces matching CoreCLR/rustc output and passes Windows x64 Native AOT. ILVerify 10.0.11 reports `ReturnPtrToStack` for reference returns, including the equivalent C# `Direct(ref Pair) => ref p.X` in Release/Debug; ILVerify acceptance remains unresolved. These results do not close enum/tag/downcast, nested references, aggregates storing references, general slice ABI or the full native platform gate. |

Local E7 verification (SDK 10.0.401): Release zero warnings/errors, 551/551 harness
tests, 24/24 `safe-core-regression-v2` and 16/16 `p1-differential-v2` cases.
The companion [storage sample](../../samples/mir-places-verified.rs) also matches
rustc 1.98.0 and passes ILVerify 10.0.11 and Windows x64 Native AOT. Reference-return
ILVerify acceptance remains unresolved as recorded in E7.
...
| ID | Status | Deliverable / ownership | Depends on | Done when | Evidence |
| --- | --- | --- | --- | --- | --- |
| P1-06.01 | ✅ Complete | Freeze the P1 executable coverage ledger — Semantics + QA | P1-04, P1-05 | List every current P1 promise as executable, check-only by existing contract, or explicitly excluded; map every executable family to a leaf below. Existing missing semantics cannot be reclassified to obtain completion. | [Frozen bilingual ledger](../p1-exit-scope-v1.md): `p1-exit-scope-v1`, 40 stable requirement IDs; case/backend denominators remain P1-10.01/.02 |
...
| P1-06.04 | 🚧 In progress | Type every place/projection — MIR model + validator | P1-06.01, P1-06.02 | Root/field/tuple/index/deref chains retain owner, result type and mutability; invalid field/index/type and depth overflow reject. | `SafeCoreMirPlaceTests`, `SafeCoreMirAdtLayoutTests`, `SafeCoreMirAdtSourceTests`: explicit named/tuple-struct layouts, nested root/field/tuple/fixed-array/deref chains, `DynamicIndex`, source-linked type/access facts and projection/work/time/cancellation bounds. Enum/tag/downcast and general slice projections remain open; E7. |
| P1-06.05 | 🚧 In progress | Reference provenance across locals, calls and joins | P1-06.04 | Every reference has an explicit source place and lifetime relation; returns, parameters and CFG joins cannot invent an origin. | `SafeCoreMirReferenceProvenanceTests`, `SafeCoreMirAdtSourceTests`: bounded CFG/call return-origin propagation, `StorageScope`, source escapes and provenance tampering; real managed references preserve caller owners. Source lifetime elision rejects absent/ambiguous input lifetimes with `RSM2004`. Nested references, aggregates storing references and general slice ABI remain open; E7. |
...
| P1-06.10 | 🚧 In progress | Pattern and match lowering, including move/ref bindings | P1-06.04, P1-06.05 | Tuple/scalar/declared ADT patterns, guards, or-patterns and exhaustiveness preserve evaluation/binding semantics; mismatched bindings reject. | `SafeCoreMirPatternExecutionTests`, `SafeCoreMirAdtSourceTests`: existing scalar/tuple match and struct/tuple-struct binding projections; the full source match/move/ref corpus remains open. |
...
| P1-06.15 | 🚧 In progress | Budgets, cancellation and fail-closed capability checks | P1-06.13 | Each new family has positive, unsupported, size/depth/work/time/cancellation tests; check/build agree and emit no partial output on failure. | `SafeCoreMirPlaceTests`, `SafeCoreMirAdtLayoutTests`, `SafeCoreMirReferenceProvenanceTests`, `SafeCoreMirAdtSourceTests`: projection/layout/provenance limits, source rejection and no-output checks; remaining family boundaries still need coverage. |
| P1-06.16 | 🚧 In progress | Deterministic MIR/LIR/PE/PDB snapshots | P1-06.14, P1-06.15 | Repeated builds of the same frozen inputs preserve bytes and ordered source/provenance facts; format changes get a new version. | `SafeCoreMirAdtSourceTests`, `SafeCoreMirReferenceProvenanceTests`: deterministic MIR layout/source snapshots and ordered return-origin facts; the full snapshot manifest and PE/PDB hash coverage remain open. |
...
| P1-06.18 | 🚧 In progress | Aggregate construction and layout — MIR/CLR value types | P1-06.01, P1-06.04 | Declared tuples, arrays and ADT variants preserve field order, discriminants, nested layout and single evaluation; malformed/recursive/oversized layouts reject. | `SafeCoreMirAdtLayoutTests`, `SafeCoreMirAdtSourceTests`, `SafeCoreMirProjectionBackendTests`: named/tuple/unit structs, nested tuple/fixed-array fields, declaration-order storage, source-order initialization, struct updates and imported constructor aliases. Enum/tag/downcast and aggregates storing references remain open; E7. |
| P1-06.19 | 🚧 In progress | Aggregate projection reads, writes and evaluation order | P1-06.18, P1-06.05 | Declared field/tuple/index/deref operations access the checked owner, evaluate receiver/index/value in order and preserve mutations; wrong owner/type/bounds fail without alias substitution. | `SafeCoreMirAdtSourceTests`, `SafeCoreMirProjectionBackendTests`: nested field/tuple/fixed-array/deref reads, projected stores/borrows, checked `DynamicIndex` reads/writes and RHS-before-place primitive compound assignments; `SafeCoreMirSliceTests` covers dynamic full-array slice reads. General slice writes and remaining value families stay open; E7. |
...
1. P1-06.01 is ✅ Complete with the frozen scope ledger. Deliver P1-10.01 and P1-10.02 next to freeze case coverage and denominators. These are finite inventory deliveries, not a rewrite of the entire design.
