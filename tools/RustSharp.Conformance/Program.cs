    ...
    public static async Task<int> Main(string[] args)
    {
        if (args.Length == 1 && args[0] is "--help" or "-h")
        {
            Console.WriteLine("""
                ...
                safe-core-regression-v3 adds the 26-case P1-06 completion regression contract without changing v2.
                ...
                p1-differential-v2 expands it to 16 fixed borrow/Drop cases, including four ownership compile-fail cases.
        ...
        if (options.Profile is SafeCoreRegressionV2ProfileRunner.ProfileName or SafeCoreRegressionV2ProfileRunner.ProfileV3Name)
        {
            try
            {
                string regressionReportPath = options.ReportPath is null
                ...
                return await SafeCoreRegressionV2ProfileRunner.RunAsync(
                    ...
                    options.Deadline,
                    startedAtUtc,
                    harnessClock, options.Profile).ConfigureAwait(false);
            }
            catch (Exception exception) when (
                exception is IOException or UnauthorizedAccessException or ArgumentException or
            ...
            {
                Console.Error.WriteLine(
                    $"conformance: {options.Profile} harness error: {TrimDiagnostic(exception.Message)}");
                return 2;
            }
        }
        ...
        if (options.Profile is P1DifferentialProfileRunner.ProfileName or P1DifferentialProfileRunner.ProfileV2Name)
        {
            try
            {
                string differentialReportPath = options.ReportPath is null
                    ? Path.Combine(repositoryRoot, "artifacts", "conformance", options.Profile + ".json")
                    : Path.GetFullPath(options.ReportPath, repositoryRoot);
                Directory.CreateDirectory(Path.GetDirectoryName(differentialReportPath)!);
                return await P1DifferentialProfileRunner.RunAsync(
                    repositoryRoot,
                    differentialReportPath,
                    options.Timeout,
                    options.Deadline,
                    startedAtUtc,
                    harnessClock,
                    options.Profile).ConfigureAwait(false);
            }
            catch (Exception exception) when (
                exception is IOException or UnauthorizedAccessException or ArgumentException or
                NotSupportedException or OperationCanceledException or TimeoutException or JsonException)
    ...
    private static Options ParseOptions(string[] args)
    {
            ...
            not SafeCoreGenericProfileRunner.ProfileName)
        {
            throw new ArgumentException(
                $"Supported profiles are '{ProfileName}', '{SafeCoreLexingProfileName}', '{SafeCoreSyntaxProfileName}', '{SafeCoreNameResolutionProfileName}', '{SafeCorePrimitivesProfileName}', '{SafeCoreTypeProfileRunner.ProfileName}', '{SafeCoreRegressionProfileRunner.ProfileName}', '{SafeCoreRegressionV2ProfileRunner.ProfileName}', '{SafeCoreRegressionV2ProfileRunner.ProfileV3Name}', '{P1ExitGateProfileRunner.ProfileName}', '{P1DifferentialProfileRunner.ProfileName}', and '{SafeCoreGenericProfileRunner.ProfileName}'.");
        }
